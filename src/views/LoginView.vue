<template>
        <v-col align="center" justify="center">
            ...
            <div class="w-full max-w-sm p-4 bg-white border border-gray-200 rounded-lg shadow sm:p-6 dark:bg-gray-800 dark:border-gray-700">
                <div v-if="wrongNetwork" @click="switchNetwork" id="alert-2" class="flex p-4 mb-4 text-red-800 rounded-lg bg-red-50 dark:bg-gray-800 dark:text-red-400" role="alert">
                    <svg aria-hidden="true" class="flex-shrink-0 w-5 h-5" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7-4a1 1 0 11-2 0 1 1 0 012 0zM9 9a1 1 0 000 2v3a1 1 0 001 1h1a1 1 0 100-2v-3a1 1 0 00-1-1H9z" clip-rule="evenodd"></path></svg>
                    <span class="sr-only">Info</span>
                    <div class="ml-3 text-sm font-medium">
                        To continue, you need to switch to the Cronos Mainnet. <br> <a>Switch?</a>
                    </div>
...
export default {
    ...
    methods:{
        authUser(walletAddress){
            const path = 'https://www.thecronosverse.com/api/ping/user'
            var obj = {
            ...
            .then(function (res) {
                ...
                if(res.data == 'No WalletAddress'){
                    $cookies.set('signupToken', walletAddress)
                    location.replace('/account/signup')
                }
                else if(res.data[2] == walletAddress){
                    var pfpLink = 'https://i.pinimg.com/736x/8f/1b/09/8f1b09269d8df868039a5f9db169a772.jpg'
                    res.data.push(pfpLink)
        ...
        onComplete(data){
            this.overlay = true;
            if(data.netID == 25){
                this.authUser(data.metaMaskAddress)
            }else{
        ...
        async connectWC(){
            try{
                const projectId = '0cd952f16349a45f0994298645a57c36'
                ...
                if (uri) {
                    web3Modal.openModal({ uri, standaloneChains: ['eip155:25'] })
                    const session = await approval();
                    var walletAddress = session.namespaces['eip155'].accounts[0]
                    var extractedWA = walletAddress.split('eip155:25:')[1]
                    this.overlay = true;
                    this.authUser(extractedWA)
